...
export default function ProductsPage() {
  ...
  const { data: productsData, isLoading } = useQuery({
    ...
    queryFn: () => getProducts({ category: selectedCategory || undefined, limit: 50 }),
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-extrabold text-stone-900">{t('products')}</h1>
        <SearchBar />
      </div>
      ...
      <nav className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 mb-6">
        {displayCats.map(cat => (
          <button
            ...
            className={
              selectedCategory === cat.categoryId || (!selectedCategory && !cat.categoryId)
                ? 'cat-pill-active'
                : 'cat-pill-inactive'
            }
          >
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={lang === 'hi' ? 'इस श्रेणी में कोई उत्पाद नहीं है।' : 'No products in this category yet.'}
        />
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
