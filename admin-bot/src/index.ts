...
bot.onText(/\/set_delivery_area|🚚 Delivery Areas/, async msg => {
  ...
  } catch { bot.sendMessage(msg.chat.id, '❌ Failed to fetch areas.'); }
});
...
bot.onText(/\/set_delivery_slots|⏰ Delivery Slots/, async msg => {
  ...
  } catch { bot.sendMessage(msg.chat.id, '❌ Failed to fetch slots.'); }
});
...
bot.onText(/\/offers|🏷️ List Offers/, async msg => {
  try {
    ...
    if (!offers || offers.length === 0) {
      bot.sendMessage(msg.chat.id, '🏷️ *No active offers right now.*\n\nUse /newoffer or "🔥 New Offer" to create one.', { parse_mode: 'Markdown' });
      return;
    }

    const TYPE_EMOJI: Record<string, string> = { flash_sale: '⚡', banner: '🎉', category_sale: '🏷️' };
    const THEME_EMOJI: Record<string, string> = { fire: '🔥', summer: '☀️', festival: '🎊', fresh: '🌿', royal: '👑' };
    ...
    const lines = offers.map((o: any, i: number) => {
      ...
      const discountStr = o.discountType === 'percentage' ? `${o.discountValue}%` : `₹${o.discountValue}`;
      const cats = o.applicableCategories?.length ? o.applicableCategories.join(', ') : 'All';
      const prods = o.applicableProducts?.length ? `${o.applicableProducts.length} products` : '';
    ...
    const buttons = offers.map((o: any) => ([
      { text: `⏹ End: ${o.title.slice(0, 20)}`, callback_data: `offer:end:${o.offerId}` },
    ...
    bot.sendMessage(msg.chat.id, `❌ Failed to fetch offers: ${(e as Error).message}`);
  }
});
...
bot.onText(/^\/quickoffer(?:$|\s+([\s\S]*))/, async (msg, match) => {
  ...
  try {
    ...
    if (!d.title || !d.discount) {
      bot.sendMessage(msg.chat.id, '❌ Title and Discount are required. Use /quickoffer to see format.');
      return;
    }
    ...
    // Parse categories
    const cats = d.categories?.toLowerCase() === 'all' || !d.categories
      ? []
...
bot.onText(/\/endoffer(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (!offerId) {
    ...
    try {
      const { data } = await axios.get(`${API}/api/offers/active`);
      if (!data.offers?.length) { bot.sendMessage(msg.chat.id, '🏷️ No active offers to end.'); return; }
      const btns = data.offers.map((o: any) => ([
        { text: `⏹ ${o.title.slice(0, 35)}`, callback_data: `offer:end:${o.offerId}` },
      ]));
      bot.sendMessage(msg.chat.id, '⏹ *Select offer to end:*', {
        parse_mode: 'Markdown',
        reply_markup: { inline_keyboard: btns },
      });
    } catch { bot.sendMessage(msg.chat.id, '❌ Failed to fetch offers.'); }
    return;
  }
  try {
    await axios.put(`${API}/api/offers/${offerId}`, { isActive: false });
    bot.sendMessage(msg.chat.id, `✅ Offer \`${offerId}\` ended.`, { parse_mode: 'Markdown', ...menu() });
  } catch { bot.sendMessage(msg.chat.id, '❌ Failed to end offer.'); }
});
...
bot.onText(/\/deloffer(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (!offerId) {
    try {
      ...
      if (!data.offers?.length) { bot.sendMessage(msg.chat.id, '🏷️ No offers to delete.'); return; }
      const btns = data.offers.slice(0, 10).map((o: any) => ([
        { text: `🗑️ ${o.title.slice(0, 35)}`, callback_data: `offer:confirmdel:${o.offerId}` },
      ]));
  ...
  try {
    ...
    bot.sendMessage(msg.chat.id, `✅ Offer \`${offerId}\` deleted.`, { parse_mode: 'Markdown', ...menu() });
  } catch { bot.sendMessage(msg.chat.id, '❌ Failed to delete offer.'); }
});
...
bot.on('callback_query', async query => {
  ...
  if (d === 'slot:list_remove') {
    ...
    } catch { bot.sendMessage(chatId, '❌ Failed.'); }
    return;
  }
  ...
  if (d.startsWith('offer:end:')) {
    ...
    try {
      await axios.put(`${API}/api/offers/${offerId}`, { isActive: false });
      bot.sendMessage(chatId, `✅ Offer \`${offerId}\` has been *ended*.`, { parse_mode: 'Markdown', ...menu() });
    } catch { bot.sendMessage(chatId, '❌ Failed to end offer.'); }
    return;
  }
  ...
  if (d.startsWith('offer:confirmdel:')) {
    ...
    bot.sendMessage(chatId, `⚠️ *Delete this offer permanently?*\n\nOffer ID: \`${offerId}\``, {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [[
  ...
  if (d.startsWith('offer:del:')) {
    ...
    try {
      await axios.delete(`${API}/api/offers/${offerId}`);
      bot.sendMessage(chatId, `✅ Offer \`${offerId}\` *deleted*.`, { parse_mode: 'Markdown', ...menu() });
    } catch { bot.sendMessage(chatId, '❌ Failed to delete offer.'); }
    return;
  }
  ...
  if (d.startsWith('offertheme:')) {
    ...
    // Build preview
    const o = state.data;
    const discStr = o.discountType === 'flat' ? `₹${o.discountValue} OFF` : `${o.discountValue}% OFF`;
...
bot.on('message', async msg => {
  ...
  if (state.step === 'add_image') {
    if (msg.photo) {
      ...
      try { state.data.imageUrl = await bot.getFileLink(photo.file_id); } catch { state.data.imageUrl = ''; }
    } else { state.data.imageUrl = ''; }
    state.step = 'add_name';
    ...
    state.data.categoryId = map[text] || text; state.step = 'add_desc';
    ...
    state.data.stock = parseInt(text); state.step = 'add_min_delivery';
    ...
    state.data.minHomeDeliveryQuantity = parseInt(text) || 0; state.step = 'add_specs';
  ...
  // Final Save — Product
    ...
    if (text.toLowerCase() === 'cancel') { wizards.delete(chatId); bot.sendMessage(chatId, '❌ Cancelled product addition.', menu()); return; }

    const parseMeta = (str: string) => {
      if (!str || str.toLowerCase() === '/skip') return {};
      const obj: Record<string, string> = {};
      str.split('\n').forEach(l => { const parts = l.split(':'); const k = parts.shift()?.trim(); const v = parts.join(':').trim(); if (k && v) obj[k] = v; });
      return obj;
    };
  ...
  // ══════════════════════════════════════════════════════════
    ...
    bot.sendMessage(chatId, 'Step 2/7: Enter offer title in *Hindi* (or /skip):', { parse_mode: 'Markdown' });
    ...
    bot.sendMessage(chatId, 'Step 3/7: Select offer *type*:', {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '🎉 Banner (all products)', callback_data: 'offertype:banner' }],
          [{ text: '🏷️ Category Sale', callback_data: 'offertype:category_sale' }],
        ],
      },
    });
    ...
    if (input.toLowerCase().includes('flat') || input.startsWith('₹')) {
      ...
      state.data.discountValue = parseFloat(input.replace(/[₹flat]/gi, '').trim());
    } else {
    ...
    state.data.categories = text.toLowerCase() === 'all' ? 'All products' : text;
    state.data.applicableCategories = text.toLowerCase() === 'all'
      ? []
    ...
    try {
      ...
      const o = data.offer;
      const discStr = o.discountType === 'percentage' ? `${o.discountValue}%` : `₹${o.discountValue}`;
      ...
      bot.sendMessage(chatId, `❌ Failed to create offer: ${(e as Error).message}`, menu());
    }
    wizards.delete(chatId);
  ...
  // ══════════════════════════════════════════════════════════
    ...
    if (['price', 'mrp', 'stock', 'minHomeDeliveryQuantity'].includes(field as string)) payload[field as string] = parseFloat(text);
    if (field === 'isActive') payload[field as string] = text.toLowerCase() === 'true';
    try {
    ...
    try {
      await axios.post(`${API}/api/delivery/charges`, { type: 'flat', flatCharge: state.data.flatCharge, freeAboveAmount: parseFloat(text) });
    ...
    try { await axios.post(`${API}/api/delivery/areas`, { pincode, name }); bot.sendMessage(chatId, `✅ Pincode ${pincode} added.`, menu()); } catch { bot.sendMessage(chatId, '❌ Failed.', menu()); }
    wizards.delete(chatId);
  } else if (state.step === 'area_remove_pin') {
    try { await axios.delete(`${API}/api/delivery/areas/${text.trim()}`); bot.sendMessage(chatId, `✅ Pincode ${text} removed.`, menu()); } catch { bot.sendMessage(chatId, '❌ Failed.', menu()); }
    wizards.delete(chatId);
    ...
    try {
      await axios.post(`${API}/api/delivery/slots`, { label: state.data.label, startTime: state.data.startTime, endTime: state.data.endTime, maxOrders: parseInt(text) });
